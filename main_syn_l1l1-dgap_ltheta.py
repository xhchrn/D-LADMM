    ...
    def forward(self, x):
        ...
        for k in range(self.layers):
            if k == 0 :
                ...
                E.append(self.self_active(X - self.A.mm(Z[-1]) - self.beta2[k].mul(self.L0), self.active_para1[k]))
                ...
                E.append(self.self_active(X - self.A.mm(Z[-1]) - self.beta2[k].mul(L[-1]), self.active_para1[k]))
                T.append(self.A.mm(Z[-1]) + E[-1] - X)
...
for epoch in range(num_epoch):
    ...
    torch.save(model.state_dict(), model.name()+'_l1l1-dgap_ltheta.pth')

    print('---------------------------testing---------------------------')
    # model.eval()
    l1l1_dgap_values = torch.zeros(layers).cuda()
    for j in range(n_test//batch_size):
        ...
        for jj in range(layers):
            l1l1_dgap_values[jj] = l1l1_dgap_values[jj] + (
    ...
    l1l1_dgap_values = l1l1_dgap_values / (n_test//batch_size)
    print('==>> epoch: {}'.format(epoch))
    for k in range(layers):
        # print('PSNR{}:{:.3f}'.format(k+1, psnr[k]), end=' ')
        print('L1L1-DGap value{}:{:.3f}'.format(k+1, l1l1_dgap_values[k]), end=' ')
    print(" ")
