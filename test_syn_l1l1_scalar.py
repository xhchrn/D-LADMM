...
log_file = os.path.join(
    'test-logs',
    'scalar-{obj}-results-{alg}-p{p}-mu{mu}-sigma{sigma}-{delta}{method}{param}.txt'.format(
        obj = objective.lower(),
        ...
        for k in range(K):
            ...
            if k == 0 :
                ...
                if use_learned:
                    ...
                    VVar = self.L0 + self.beta2[k] * (self.A.mm(Zn_L2O) + self.E0 - X)
                ...
                if use_learned:
                    ...
                    # Step 2 NOTE: my modification
                    VVar = L[-1] + self.beta2[k] * (self.A.mm(Zn_L2O) + E[-1] - X)
