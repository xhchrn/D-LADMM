    ...
    def forward(self, x):
        ...
        for k in range(self.layers):
            if k == 0 :
                ...
                E.append(self.ss2_1[k].mul(residual) - self.ss2_2[k].mul(self.L0))
                # Step 3
                ...
                residual = X - self.A.mm(Z[-1])
                E.append(self.ss2_1[k].mul(residual) - self.ss2_2[k].mul(L[-1]))
...
if __name__ == '__main__':
    ...
    syn_data = sio.loadmat('syn_data_lasso.mat')
    A_ori = syn_data['A']
    ...
    for epoch in range(num_epoch):
        ...
        for j in range(n//batch_size):
            ...
            for k in range(layers):
                ...
                loss.append(
                    ...
                    0.5 * torch.sum((input_bs_var - torch.mm(A_tensor, Z[k]))**2.0, dim=0).mean()
                    # lam * sl2_loss
                )
        ...
        torch.save(model.state_dict(), model.name()+'_lasso_scalar_alpha{}.pth'.format(alpha))

        print('---------------------------testing---------------------------')
        # model.eval()
        lasso_values = torch.zeros(layers).cuda()
        for j in range(n_test//batch_size):
            ...
            for jj in range(layers):
                ################ lasso_values[jj] = lasso_values[jj] + F.mse_loss(255 * input_gt_var.cuda(), 255 * torch.mm(A_tensor, Z[jj]), reduction='elementwise_mean')
                ################ lasso_values[jj] = lasso_values[jj] + F.mse_loss(255 * input_gt_var.cuda(), 255 * torch.mm(A_tensor, Z[jj]))
                ################ lasso_values[jj] = lasso_values[jj] + ((255 * input_gt_var.cuda() - 255 * torch.mm(A_tensor, Z[jj]))**2).mean()
                ################ lasso_values[jj] = lasso_values[jj] + (alpha * torch.mean(torch.abs(Z[jj])) + torch.mean(torch.abs(E[jj])))
                # NOTE: TODO: Add normalization after talking with Howard
                lasso_values[jj] += alpha * torch.sum(torch.abs(Z[jj])) + \
        ...
        for k in range(layers):
            ...
            print('Loss{}:{:.3f}'.format(k+1, lasso_values[k]), end=' ')
        print(" ")
