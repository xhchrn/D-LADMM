...
log_file = os.path.join(
    'newS-test-logs',
    'scalar-ptied-layers{layers}-{obj}-alpha{alpha}-results-{alg}{continued}-cols{cols}-p{p}-mu{mu}-sigma{sigma}-{data_type}-{delta}{method}{param}.txt'.format(
        ...
        for k in range(K):
            ...
            if k == 0 :
                ...
                if use_learned:
                    ...
                    Zn_L2O = self.self_active(self.Z0 - self.ss1[k] * fc(Varn_L2O.t()).t(), self.active_para[k])
                ...
                if use_learned:
                    ...
                    Zn_L2O = self.self_active(Z[-1] - self.ss1[k] * fc(Varn_L2O.t()).t(), self.active_para[k])
